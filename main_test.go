...
func TestLoop42(t *testing.T) {
	...
	if p1.Action.Name != constant.AllIn {
		t.Error()
	}
	if !state.Snapshot.Gambit.Bet(p2.ID, 10) {
		t.Error()
	}
	if p2.Action.Name != constant.AllIn {
		t.Error()
	}
	if !state.Snapshot.Gambit.Call(p3.ID) {
		t.Error()
	}
	if p3.Action.Name != constant.AllIn {
		t.Error()
	}
	if !state.Snapshot.Gambit.Raise(p4.ID, 30) {
		t.Error()
	}
	if p4.Action.Name != constant.AllIn {
		t.Error()
	}
	// p1.Print()
	// p2.Print()
...
func TestLoop48(t *testing.T) {
	...
	if !state.Snapshot.Gambit.Check(b.ID) {
		t.Error()
	}
	if !state.Snapshot.Gambit.Check(c.ID) {
		t.Error()
	}
	...
	// handler.Sit("b", 5)
	// handler.Sit("c", 1)
	if len(others[1].Cards) > 0 {
		t.Error()
	}
	if tmp := others[2]; len(tmp.Cards) > 0 && !tmp.IsWinner {
		t.Error()
	}
	if tmp := others[2]; len(tmp.Cards) < 3 && tmp.IsWinner {
		t.Error()
	}
	if tmp := others[5]; len(tmp.Cards) < 3 {
		t.Error()
	}
	handler.Connect("d")
	handler.Sit("d", 0)
	d := &state.Snapshot.Players[0]
	a.Chips = 250
	b.Chips = 300
	c.Chips = 250
	d.Chips = 250
	...
	if !state.Snapshot.Gambit.Check(d.ID) {
		t.Error()
	}
	if !state.Snapshot.Gambit.NextRound() {
		t.Error()
	...
	if !state.Snapshot.Gambit.Call(d.ID) {
		t.Error()
	}
	if !state.Snapshot.Gambit.Raise(a.ID, 150) {
		t.Error()
	}
	if !state.Snapshot.Gambit.Call(b.ID) {
		t.Error()
...
func TestLoop53(t *testing.T) {
	...
	for _, player := range state.Snapshot.Players {
		if player.ID != "" && player.Actions[len(player.Actions)-2].Name != constant.Stand {
			t.Error()
		}
	}
	...
	b.Cards = model.Cards{43, 19, 31} // Q69 spades
...
func TestLoop54(t *testing.T) {
	t.Run("Q69 spades < Q79 clubs", func(t *testing.T) {
		...
		c, ckind := gambit.NineK{}.Evaluate(model.Cards{
			40, 20,
		})
		d, _ := gambit.NineK{}.Evaluate(model.Cards{
			40,
		})
		if akind != constant.Flush || bkind != constant.Flush || ckind != constant.Flush {
			t.Error()
		}
		if len(a) != 5 || len(b) != 5 || len(c) != 5 || len(d) != 5 {
			t.Error()
		}
	})
}

func TestLoop55(t *testing.T) {
	decisionTime := int64(1)
	ninek := gambit.NineK{
		MaxAFKCount:     5,
		FinishGameDelay: 5,
	...
	handler.Sit("a", 5)
	handler.Sit("b", 2)
	a := &state.Snapshot.Players[5]
	b := &state.Snapshot.Players[2]
	if a.Actions[len(a.Actions)-1].Hints[1].Value != 500 {
		t.Error()
	}
...
func TestLoop58(t *testing.T) {
	...
	if len(state.Snapshot.TempHistory["a"].Competitors) != 2 {
		t.Error()
	}
	if len(state.Snapshot.TempHistory["b"].Competitors) != 2 {
		t.Error()
	}
	if len(state.Snapshot.TempHistory["c"].Competitors) != 2 {
		t.Error()
	}
	if !state.Snapshot.Gambit.Check("a") {
		t.Error()
...
func TestLoop59(t *testing.T) {
	...
	if len(state.Snapshot.TempHistory["a"].Competitors[0].Cards) != 0 {
		t.Error()
	}
